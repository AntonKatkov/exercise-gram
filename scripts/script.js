...
function newPostRender() {
    ...
    for (let i = 0; i < userInfos.length; i++) {
        ...
        for (let x = 0; x < user['comments'].length; x++) {
            ...
            usercommentsTextLength.innerHTML += Usercomments(user, i, x, comment);
        }
    }
...
function openFullscreen(imagePath, index, userPostText, userAvatar, userAuthor) {
    ...
    if (!modal) {
        // Modal existiert nicht, also erstelle es dynamisch
        modal = document.createElement("div");
        modal.id = modalId;
        modal.className = "modalx";
        // Verwende imagePath direkt für das Bild im Modal
